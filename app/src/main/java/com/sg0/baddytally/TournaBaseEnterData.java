...
public class TournaBaseEnterData extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private void fetchGames() {
        //mGameNum = 1;
        Log.i(TAG, "fetchGames..." + mMatchId);
        DatabaseReference dbRef = mDatabase.child(mCommon.mClub).child(Constants.TOURNA)
                .child(mCommon.mTournament).child(Constants.MATCHES).child(mMatchId);
        ...
        dbRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                mMatchDBEntry = dataSnapshot.getValue(TournaFixtureDBEntry.class);
    ...
    private Boolean isMatchDone(final ArrayList<GameJournalDBEntry> newGameList, final Boolean setWinner) {
        ...
        if (newGameList == null || newGameList.size() == 0) return false;
    ...
    private void workToUpdateDB() {
        ...
        if (!SharedData.getInstance().isDBLocked()) {
            Toast.makeText(TournaBaseEnterData.this, "Another update is in progress, try again later...", Toast.LENGTH_LONG).show();
        ...
        if (checkbox.isChecked() && !winner.isEmpty()) {
            ...
            mMatchDBEntry.setT1(mTeams.get(0));
            mMatchDBEntry.setT2(mTeams.get(1));
        ...
        /*
        for (GameJournalDBEntry jEntry : mGameList) {
            Log.i(TAG, "WRITING jEntry: " + jEntry.toReadableString());

            final DatabaseReference jDBEntryRef = mDatabase.child(mCommon.mClub).child(Constants.TOURNA)
    ...
    private void releaseLockAndCleanup(final Boolean finish, final Boolean dbUpdated) {
